...
(function( $ ){
  ...
  var grid = "galleryGrid",
      ...
      defaults = {
        ...
        selectRight: ".right",
        selectItem: ".item",
        selectFilterList: ".filter-list",
        selectFilterBtn: "#mbr-filter",
        selectFilterBtnAccept: "#mbr-filter-accept",
  ...
  function Grid(element, options){
    ...
    self._name = grid;
    self.items = $(self.opt.selectItem);
    self.wraps = [];
    self.wrapUpdateFlag = true;
    self.filterList = $(self.opt.selectFilterList);
    self.filter = [];
    self.updateNow = true;
    self.width = 0;
    // self.filterUpdateFlag = false;
    self.filtered = $(self.opt.selectItem);
    self.init();
  }

  function screenType(){
    var w = $(window).width();
    if(w >= self.opt.tabletStarts){
      return "desktop";
    }
    if(w < self.opt.tabletStarts && w >= self.opt.mobileStarts){
      return "tablet";
    }
    return "mobile";
  }
  ...
  Grid.prototype.evalSizes = function(){
    ...
    var ww = self.width - self.opt.bodyMargins*2;
    switch(screenType()){
      case "desktop" :
        // self.width = self.width - (self.width % 4); //люфт 3 пикселя 
        self.X = (ww - 4*self.opt.margin)/2; 
        self.Y = ww/4 - 2*self.opt.margin;
        break;
      case "tablet" :
        // self.width = self.width - (self.width % 4); //люфт 3 пикселя 
        self.X = (ww - 4*self.opt.margin)/2; 
        ...
        break;
      case "mobile" :
        self.X = (ww); //change to correct body margins on mobile
        self.Y = self.X;
        // console.log(self.X);
        break;
      default : console.log("unknown screen type");
    }
  }
  ...
  Grid.prototype.setMargins = function (){
    if( self.opt.addGalleryMargin ){
      self.element
        .css("margin", Math.round(self.opt.margin)+"px");
    }
    $(self.filtered)
      .css("margin", Math.round(self.opt.margin)+"px");
  }

  Grid.prototype.wrap = function(){
    if(self.wraps.length > 0){
      // self.element.append(self.items);
      // for (var t = 0 ; i < self.wraps.length; t++){
      //   self.wraps[t].remove();
      // }
      self.items.unwrap();
      self.wraps = [];
    ...
    var n = self.filtered.length, block_size, temp_mass=[], wraps_count, index;
    // self.wraps.push($("<div />",{
    //   "class": self.opt.selectFirst.slice(1)
    // }));
    // self.element.append(self.wraps[0]);
    // self.wraps[0].append(self.items[0]);
    wraps_count = (n <= 4)? 1 : 1 + Math.ceil((n-4)/3);
    for(var i = 0; i < wraps_count; i++){
      if( i === 0){
        self.wraps.push($("<div />",{
          "class": self.opt.selectFirst.slice(1)
        }));
        ...
        switch(i % 3){
          case 0:
            self.wraps.push($("<div />",{
              "class": self.opt.selectRight.slice(1)
            }));
            break;
          case 1:
            self.wraps.push($("<div />",{
              "class": self.opt.selectCenter.slice(1)
            }));
            break;
          case 2:
            self.wraps.push($("<div />",{
              "class": self.opt.selectLeft.slice(1)
            }));
            break;
          default: alert("Fatal error in Grid.prototype.wrap switch!");
        }
      }
    }
    self.element.append(self.wraps);
    for ( i = 0; i < wraps_count; i++){
      if( i == wraps_count - 1){
        index = i > 0? 4+3*(i-1) : 0;
        self.wraps[i].append(self.filtered.slice(index));
      } else{
        if(i == 0){
          self.wraps[i].append(self.filtered.slice(0,4));
        }
        else{
          self.wraps[i].append(self.filtered.slice(4 + 3*(i-1),4 + 3*i));
        }
      }
    }
    self.wrapUpdateFlag = false;
  }
  Grid.prototype.applyFilter = function(){
    // console.log(self.filterList);
    // console.log(self.filter);
    self.filtered = null;
    // console.log(self.filterUpdateFlag);
    self.filtered = _.filter(self.items, function(obj){
      ...
      return (_.intersection($(obj).attr("data-category").split(" "), self.filter).length > 0);
    });
  ...
  Grid.prototype.update = function(){
    self.width = Math.round($(window).width());
    self.width = self.width - (self.width % 4);
    self.X = 0;
    self.Y = 0;
    // if(self.filterUpdateFlag){
    //   self.applyFilter();
    // }
    if( screenType() === "mobile"){
      ...
      self.updateNow = true;
      self.filterList.slideDown(200);
    }

    if(self.wrapUpdateFlag){
      self.wrap(); //+
    }
    self.setMargins();
    self.evalSizes();
  ...
  Grid.prototype.onresize = function(){
    ...
    $(window).on("resize", function(){
      clearTimeout(timer);
      timer = setTimeout(self.update, 500);
    });
  }
  Grid.prototype.onfilter = function(){
    self.filterList.find("[data-category]").on("click",function(){
      if($(this).hasClass("active")){
        $(this).removeClass("active");
        self.filter = _.difference(self.filter, [$(this).attr("data-category")]);
      } else{
        $(this).addClass("active");
        self.filter = _.union(self.filter, [$(this).attr("data-category")]);
      }
      if(self.updateNow){
        self.applyFilter();
      }
  ...
  Grid.prototype.mobile_init = function(){
    self.updateNow = false;
    $(self.opt.selectFilterBtn).on("click", function(){
      self.filterList.slideToggle(200);
    });
    $(self.opt.selectFilterBtnAccept).on("click", function(){
      self.applyFilter();
      // self.update();
      self.filterList.slideUp(200);
    });
  }
...
(function( $ ){
  ...
  var defaults = {
    n : 3,
    selectItem: ".item"
  }, current, columns = [], items;

  $.fn.columnGrid = function( options ) {
    self = this;
    var opt = $.extend( {
      n: 3,
      selectItem: ".item"
    ...
    items = this.find(opt.selectItem);
    for (var i = 0; i < opt.n; i++){
      columns.push($("<div></div>").addClass("column").css("width",Math.floor(100/opt.n)+"%"));
      this.append(columns[i]);
    }
    items.each(function(index, el){
      $(this).detach().appendTo(columns[index % opt.n]);
    });
    self.reload = function(opti){
      if (opti.n && (opti.n != opt.n)){
        items.detach().appendTo(self);
        columns=[];
        for (var i = 0; i < opti.n; i++){
          columns.push($("<div></div>").addClass("column").css("width",Math.floor(100/opti.n)+"%"));
          this.append(columns[i]);
        }
        items.each(function(index, el){
          $(this).detach().appendTo(columns[index % opti.n]);
        });
      }
    }
